# servers/mcp/dataiku_prompt_functions.py
import logging
from typing import Dict, Any, Optional

# Centralized prompt storage for Dataiku MCP Server
PROMPT_TEMPLATES = {
    "instructions_prefix": """
...
def get_prompt(args: Dict[str, Any], sly_data: Dict[str, Any]) -> str:
    """
    Retrieve and optionally format a prompt template.
    :param prompt_name: The name of the prompt template to retrieve.
    :param format_params: Optional parameters for formatting the prompt template.
    """
    prompt_name = args.get("prompt_name", "")
    format_params = args.get("format_params", {})
    
    logging.info(f"[MCP-Prompt] Retrieving prompt '{prompt_name}' with params: {format_params}")
    ...
    if "{instructions_prefix}" in prompt and "instructions_prefix" not in format_params:
        format_params["instructions_prefix"] = PROMPT_TEMPLATES["instructions_prefix"]
...
def list_prompts(args: Dict[str, Any], sly_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Return information about all available prompts.
    """
    logging.info("[MCP-Prompt] Listing all available prompts")
...
def format_prompt(args: Dict[str, Any], sly_data: Dict[str, Any]) -> Dict[str, str]:
    """
    Format a prompt template with provided parameters.
    :param prompt_name: The name of the prompt template to format.
    :param parameters: Dictionary of parameters for formatting.
    """
    prompt_name = args.get("prompt_name", "")
    parameters = args.get("parameters", {})
    
    logging.info(f"[MCP-Prompt] Formatting prompt '{prompt_name}' with parameters: {parameters}")
    ...
    # Auto-format prompts that contain instructions_prefix placeholder
    if "{instructions_prefix}" in prompt and "instructions_prefix" not in parameters:
        parameters["instructions_prefix"] = PROMPT_TEMPLATES["instructions_prefix"]
...
# Registry dictionary following the same pattern as tools
